...
from fastapi import FastAPI, Depends, HTTPException, status, UploadFile, Header
...
async def create_parcel(
    ...
    print(f"✓ Parcel marked: {db_parcel.parcel_name} at ({parcel.centroid_lat}, {parcel.centroid_lon})")
...
async def update_parcel(
    ...
    print(f"✓ Parcel updated: {db_parcel.parcel_name} (id={db_parcel.id}, farmer={farmer.email})")
    return db_parcel
...
async def delete_parcel(
    ...
    print(f"✓ Parcel deleted: {db_parcel.parcel_name} (parcel_id={parcel_id})")
...
async def get_wms_tiles(z: int, x: int, y: int):
    ...
    return {
        ...
        "attribution": "© Turkish Land Registry Directorate (TKGM)"
    }
...
async def api_info():
    ...
    return {
        ...
        "database": "SQLite (MVP) → PostgreSQL+PostGIS (Phase 2)",
